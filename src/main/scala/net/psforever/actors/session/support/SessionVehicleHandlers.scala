  ...
  def handle(toChannel: String, guid: PlanetSideGUID, reply: VehicleResponse.Response): Unit = {
    ...
    reply match {
      case VehicleResponse.VehicleState(
        ...
        sendResponse(
          VehicleStateMessage(vehicleGuid, unk1, pos, orient, vel, unk2, unk3, unk4, wheelDirection, unk5, unk6)
        )
        player.Position = pos
      ...
      case VehicleResponse.VehicleState(
        ...
        sendResponse(
          VehicleStateMessage(vehicleGuid, unk1, pos, ang, vel, unk2, unk3, unk4, wheelDirection, unk5, unk6)
        )
      ...
      case VehicleResponse.ChangeAmmo(weapon_guid, weapon_slot, previous_guid, ammo_id, ammo_guid, ammo_data)
          if isNotSameTarget =>
      ...
      case VehicleResponse.StowEquipment(vehicleGuid, slot, itemType, itemGuid, itemData) if isNotSameTarget =>
        ...
        sendResponse(
          ObjectCreateDetailedMessage(itemType, itemGuid, ObjectCreateMessageParent(vehicleGuid, slot), itemData)
        )
      ...
      case VehicleResponse.TransferPassengerChannel(oldChannel, tempChannel, vehicle, vehicleToDelete)
          if isNotSameTarget =>
      ...
      case VehicleResponse.StartPlayerSeatedInVehicle(vehicle, _) if player.VisibleSlots.contains(player.DrawnSlot) =>
      ...
      case VehicleResponse.ServerVehicleOverrideStart(vehicle, _) =>
        ...
        sessionData.vehicles.ServerVehicleOverrideWithPacket(
          ...
          ServerVehicleOverrideMsg(
            ...
            reverse = false,
            unk4 = false,
            lock_vthrust = if (GlobalDefinitions.isFlightVehicle(vdef)) { 1 }
            else { 0 },
            lock_strafe = 0,
