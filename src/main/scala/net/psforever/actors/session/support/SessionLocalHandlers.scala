    ...
    reply match {
      ...
      case LocalResponse.EliminateDeployable(obj: TurretDeployable, dguid, pos, _) =>
        ...
        DeconstructDeployable(
          ...
          pos,
          obj.Orientation,
          deletionType = if (obj.MountPoints.isEmpty) { 2 }
          else { 1 }
        )
      ...
      case LocalResponse.LluDespawned(lluGuid, position) =>
        ...
        sessionData.specialItemSlotGuid.collect {
          case guid if guid == lluGuid =>
      ...
      case LocalResponse.RechargeVehicleWeapon(vehicleGuid, weaponGuid) if resolvedPlayerGuid == guid =>
        continent
          ...
          .collect {
            case weapon: Tool if weapon.GUID == weaponGuid =>
  ...
  def HackObject(targetGuid: PlanetSideGUID, unk1: Long, unk2: Long): Unit = {
    sendResponse(
      HackMessage(
        unk1 = 0,
        targetGuid,
        player_guid = Service.defaultPlayerGUID,
        progress = 100,
        unk1,
        HackState.Hacked,
        unk2
