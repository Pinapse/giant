...
private[support] class WeaponAndProjectileOperations(
  ...
  var prefire: mutable.Set[PlanetSideGUID] =
  ...
  def handleLongRangeProjectileState(pkt: LongRangeProjectileInfoMessage): Unit = {
    ...
    FindContainedWeapon match {
      case (Some(_: Vehicle), weapons) if weapons.exists { _.GUID == guid } => ; //now what?
      case _                                                                => ;
    }
  }
      ...
      case (Some(obj), Some(tool)) =>
        ...
        if (distanceToOwner <= acceptableDistanceToOwner) {
          ...
          val mountedIn = (continent.turretToWeapon
            ...
            case Some((_, turretGuid)) =>
              Some(
                (
                  turretGuid,
                  continent.GUID(turretGuid).collect {
                    case o: PlanetSideGameObject with FactionAffinity => SourceEntry(o)
                  }
                )
              )
            case _ => None
  ...
  def HandleDamageProxyLittleBuddyExplosion(proxy: Projectile, orientation: Vector3, distance: Float): Unit = {
    ...
    val explosionFunc: () => Unit =
      WeaponAndProjectileOperations.detonateLittleBuddy(continent, obj, proxy, proxy.owner)
...
object WeaponAndProjectileOperations {
  ...
  private def littleBuddyExplosionDamage(
    ...
    DamageInteraction(
      SourceEntry(target),
      OicwLilBuddyReason(owner, projectileId, target.DamageModel),
      explosionPosition
    )
  }
}
