            ...
            case Some((ourMember, ourIndex)) =>
              ...
              membershipPositions.foreach {
                case (member, index) =>
                  ...
                  squadUI(member.CharId) = SquadUIElement(
                    member.Name,
                    outfit = 0L,
                    index,
                    member.ZoneId,
              ...
              GiveSquadColorsToMembers(
                membershipPositions.map {
                  case (member, index) =>
                    ...
                    sendResponse(
                      SquadMemberEvent
                        .Add(squad_supplement_id, charId, index, member.Name, member.ZoneId, outfit_id = 0)
                    )
                    squadUI(charId) = SquadUIElement(
                      member.Name,
                      outfit = 0L,
                      index,
                      member.ZoneId,
          ...
          val updatedEntries = pairedEntries
            .collect({
              case (entry, element) if entry.zone_number != element.zone =>
                ...
                squadUI(entry.char_id) = SquadUIElement(
                  element.name,
                  element.outfit,
                  element.index,
                  entry.zone_number,
        ...
        case SquadResponse.SquadSearchResults(results) =>
          ...
          if (results.nonEmpty) {
            results.foreach { guid =>
              sendResponse(
                SquadDefinitionActionMessage(guid, 0, SquadAction.SquadListDecorator(SquadListDecoration.SearchResult))
              )
            }
          } else {
  ...
  def SwapSquadUIElements(squad: Squad, fromIndex: Int, toIndex: Int): Unit = {
    if (squadUI.nonEmpty) {
      ...
      if (toCharId > 0) {
        ...
        squadUI(toCharId) = SquadUIElement(
          fromElem.name,
          fromElem.outfit,
          toIndex,
          fromElem.zone,
        ...
        squadUI(fromCharId) = SquadUIElement(
          toElem.name,
          toElem.outfit,
          fromIndex,
          toElem.zone,
        ...
        squadUI(fromCharId) =
          SquadUIElement(elem.name, elem.outfit, toIndex, elem.zone, elem.health, elem.armor, elem.position)
        ...
        sendResponse(
          SquadState(
            PlanetSideGUID(id),
            List(
              SquadStateInfo(fromCharId, elem.health, elem.armor, elem.position, 2, 2, unk6 = false, 429, None, None)
            )
          )
        )
      }
