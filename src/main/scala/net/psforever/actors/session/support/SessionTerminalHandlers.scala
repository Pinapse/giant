...
class SessionTerminalHandlers(
  ...
  def handleProximityTerminalUse(pkt: ProximityTerminalUseMessage): Unit = {
    ...
    continent.GUID(objectGuid) match {
      ...
      case Some(obj) =>
        log.warn(
          s"ProximityTerminalUse: ${obj.Definition.Name} guid=${objectGuid.guid} is not ready to implement proximity effects"
        )
      case None =>
      ...
      case _ =>
        ...
        continent.GUID(terminal) match {
          case Some(term: Terminal) =>
            log.warn(
              s"NoDeal?: ${tplayer.Name} made a request but the ${term.Definition.Name}#$terminal rejected the missing order"
            )
          case Some(_) =>
  ...
  /**
    ...
    */
  private[session] def registerVehicleFromSpawnPad(
      vehicle: Vehicle,
      pad: VehicleSpawnPad,
      terminal: Terminal
  ): TaskBundle = {
  ...
  def CancelAllProximityUnits(guid: PlanetSideGUID): Unit = {
    continent.GUID(guid).collect {
      case terminal: Terminal with ProximityUnit =>
        FindProximityUnitTargetsInScope(terminal)
          .foreach(target => terminal.Actor ! CommonMessages.Unuse(player, Some(target)))
