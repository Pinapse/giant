...
class SessionMountHandlers(
  ...
  def handle(tplayer: Player, reply: Mountable.Exchange): Unit = {
    reply match {
      ...
      case Mountable.CanMount(obj: Vehicle, seatNumber, _) if obj.Definition == GlobalDefinitions.orbital_shuttle =>
      ...
      case Mountable.CanMount(obj: Vehicle, seatNumber, _) if obj.Definition == GlobalDefinitions.ant =>
        sessionData.zoning.CancelZoningProcessWithDescriptiveReason("cancel_mount")
      ...
      case Mountable.CanMount(obj: Vehicle, seatNumber, _) if obj.Definition == GlobalDefinitions.quadstealth =>
        sessionData.zoning.CancelZoningProcessWithDescriptiveReason("cancel_mount")
        log.info(s"${player.Name} mounts the driver seat of the ${obj.Definition.Name}")
        val obj_guid: PlanetSideGUID = obj.GUID
        sessionData.terminals.CancelAllProximityUnits()
      ...
      case Mountable.CanMount(obj: Vehicle, seatNumber, _) if seatNumber == 0 && obj.Definition.MaxCapacitor > 0 =>
        sessionData.zoning.CancelZoningProcessWithDescriptiveReason("cancel_mount")
        log.info(s"${player.Name} mounts the driver seat of the ${obj.Definition.Name}")
        val obj_guid: PlanetSideGUID = obj.GUID
        sessionData.terminals.CancelAllProximityUnits()
      ...
      case Mountable.CanMount(obj: Vehicle, seatNumber, _) if seatNumber == 0 =>
        sessionData.zoning.CancelZoningProcessWithDescriptiveReason("cancel_mount")
        log.info(s"${player.Name} mounts the driver seat of the ${obj.Definition.Name}")
        val obj_guid: PlanetSideGUID = obj.GUID
        sessionData.terminals.CancelAllProximityUnits()
      ...
      case Mountable.CanMount(obj: Vehicle, seatNumber, _) if obj.Definition.MaxCapacitor > 0 =>
        sessionData.zoning.CancelZoningProcessWithDescriptiveReason("cancel_mount")
        log.info(s"${player.Name} mounts ${obj.SeatPermissionGroup(seatNumber) match {
      ...
      case Mountable.CanMount(obj: Vehicle, seatNumber, _) =>
        sessionData.zoning.CancelZoningProcessWithDescriptiveReason("cancel_mount")
        log.info(s"${player.Name} mounts the ${obj.SeatPermissionGroup(seatNumber) match {
          case Some(seatType) => s"a $seatType seat (#$seatNumber)"
          case None           => "a seat"
        }} of the ${obj.Definition.Name}")
        val obj_guid: PlanetSideGUID = obj.GUID
      ...
      case Mountable.CanMount(obj: FacilityTurret, seatNumber, _) if !obj.isUpgrading =>
        sessionData.zoning.CancelZoningProcessWithDescriptiveReason("cancel_mount")
        log.info(s"${player.Name} mounts the ${obj.Definition.Name}")
        sendResponse(PlanetsideAttributeMessage(obj.GUID, attribute_type = 0, obj.Health))
        sessionData.updateWeaponAtSeatPosition(obj, seatNumber)
      ...
      case Mountable.CanDismount(obj: Vehicle, seatNum, _) if obj.Definition == GlobalDefinitions.orbital_shuttle =>
        ...
        events ! VehicleServiceMessage(
          ...
          VehicleAction.SendResponse(
            ...
            PlayerStateShiftMessage(
              ShiftState(unk = 0, obj.Position, obj.Orientation.z, vel = None)
            ) //cower in the shuttle bay
          )
        )
        events ! VehicleServiceMessage(
          continent.id,
      ...
      case Mountable.CanDismount(obj: Vehicle, seatNum, _) if obj.Definition == GlobalDefinitions.droppod =>
        log.info(s"${tplayer.Name} has landed on ${continent.id}")
        sessionData.unaccessContainer(obj)
        DismountAction(tplayer, obj, seatNum)
        obj.Actor ! Vehicle.Deconstruct()

      case Mountable.CanDismount(obj: Vehicle, seatNum, _) if tplayer.GUID == player.GUID =>
        //disembarking self
      ...
      case Mountable.CanNotDismount(obj, seatNum) =>
        log.warn(
          s"DismountVehicleMsg: ${tplayer.Name} attempted to dismount $obj's mount $seatNum, but was not allowed"
        )
    }
  }
  ...
  def DismountVehicleAction(tplayer: Player, obj: PlanetSideGameObject with Mountable, seatNum: Int): Unit = {
    ...
    obj match {
      case v: Vehicle if seatNum == 0 && Vector3.MagnitudeSquared(v.Velocity.getOrElse(Vector3.Zero)) > 0f =>
