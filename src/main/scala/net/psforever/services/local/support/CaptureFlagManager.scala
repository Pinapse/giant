...
class CaptureFlagManager(zone: Zone) extends Actor {
  ...
  def receive: Receive = {
    ...
    case CaptureFlagManager.Captured(flag: CaptureFlag) =>
      ...
      flag.Zone.LocalEvents ! LocalServiceMessage(
        flag.Zone.id,
        LocalAction.TriggerSound(
          PlanetSideGUID(-1),
          TriggeredSound.LLUInstall,
          flag.Target.CaptureTerminal.get.Position,
          20,
          0.8000001f
        )
      )
      ...
      ChatBroadcast(
        flag.Zone,
        CaptureFlagChatMessageStrings.CTF_Success(name, flag.Faction, flag.Owner.asInstanceOf[Building].Name)
      )
      // Despawn flag
    ...
    case CaptureFlagManager.Lost(flag: CaptureFlag, reason: CaptureFlagLostReasonEnum) =>
      reason match {
        case CaptureFlagLostReasonEnum.Resecured =>
          ChatBroadcast(
            flag.Zone,
            CaptureFlagChatMessageStrings.CTF_Failed_SourceResecured(
              flag.Owner.asInstanceOf[Building].Name,
              flag.Faction
            )
          )
    ...
    case CaptureFlagManager.PickupFlag(flag: CaptureFlag, player: Player) =>
      ...
      continent.LocalEvents ! LocalServiceMessage(
        continent.id,
        LocalAction.SendPacket(ObjectAttachMessage(player.GUID, flag.GUID, 252))
      )
      continent.LocalEvents ! LocalServiceMessage(
        continent.id,
        LocalAction.TriggerSound(PlanetSideGUID(-1), TriggeredSound.LLUPickup, player.Position, 15, volume = 0.8f)
      )
      ChatBroadcast(
        flag.Zone,
        CaptureFlagChatMessageStrings
          .CTF_FlagPickedUp(player.Name, player.Faction, flag.Owner.asInstanceOf[Building].Name),
        fanfare = false
      )
    ...
    case CaptureFlagManager.DropFlag(flag: CaptureFlag) =>
      flag.Carrier match {
        case Some(player: Player) =>
          ...
          flag.Zone.LocalEvents ! LocalServiceMessage(
            flag.Zone.id,
            LocalAction.SendPacket(ObjectDetachMessage(player.GUID, flag.GUID, player.Position, 0, 0, 0))
          )
          // Send dropped chat message
          ChatBroadcast(
            flag.Zone,
            CaptureFlagChatMessageStrings
              .CTF_FlagDropped(player.Name, player.Faction, flag.Owner.asInstanceOf[Building].Name),
            fanfare = false
          )
        case _ =>
  ...
  private def TrackFlag(flag: CaptureFlag): Unit = {
    ...
    if (mapUpdateTick.isCancelled) {
      ...
      mapUpdateTick =
        context.system.scheduler.scheduleAtFixedRate(0 seconds, 1 second, self, CaptureFlagManager.MapUpdate())
    }
  }
  ...
  private def HandleFlagDespawn(flag: CaptureFlag): Unit = {
    ...
    zone.LocalEvents ! LocalServiceMessage(
      zone.id,
      LocalAction.LluDespawned(PlanetSideGUID(-1), flag.GUID, flag.Position)
    )
...
object CaptureFlagManager {
  ...
  final case class SpawnCaptureFlag(
