...
/**
  * ...
  */
class BfrFlightControl(vehicle: Vehicle) extends BfrControl(vehicle) with VehicleCapacitance {
        ...
        case BfrFlight.Soaring(flightValue) =>
          ...
          if (vehicle.Capacitor > 0) {
            ...
            val (_, cdrain) = if (flightValue == 0 || flightValue == -0) {
              ...
              }) definition.CapacitorDrainSpecial
              else 0
              (vdrain, vdrain + hdrain)
            }
            flying = Some(if (cdrain > 0) {
              val modDrain =
                math.max(1, (cdrain * vehicle.SubsystemStatusMultiplier(sys = "BattleframeFlightPod.UseRate")).toInt)
