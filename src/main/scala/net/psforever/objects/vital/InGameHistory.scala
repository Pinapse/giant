...
final case class HealFromTerm(term: AmenitySource, amount: Int) extends HealingActivity

final case class HealFromImplant(implant: ImplantType, amount: Int) extends HealingActivity

final case class RepairFromExoSuitChange(exosuit: ExoSuitType.Value, amount: Int) extends RepairingActivity

final case class RepairFromKit(kit_def: KitDefinition, amount: Int) extends RepairingActivity()
...
final case class RepairFromAmenityAutoRepair(amount: Int) extends RepairingActivity
...
object InGameHistory {
  def SpawnReconstructionActivity(
    ...
    val event: GeneralActivity =
      if (
        obj.History.nonEmpty || obj.History.headOption.exists {
